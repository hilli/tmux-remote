                            ...
                            struct tmuxremote_client_config* config)
{
    ...
    if (!tmuxremote_terminal_set_raw(&savedTermios)) {
    ...
    if (pthread_create(&readerThread, NULL, stream_reader_thread, &readerCtx) != 0) {
        tmuxremote_terminal_restore(&savedTermios);
    ...
    while (!atomic_load(&done)) {
        ...
        if (sigwinch_received) {
            ...
            if (tmuxremote_terminal_get_size(&newCols, &newRows)) {
                tmuxremote_coap_resize(conn, client, newCols, newRows);
            }
        }
