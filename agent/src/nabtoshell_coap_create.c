...
{
    const char* paths[] = {"terminal", "create", NULL};
                           ...
                           NabtoDeviceCoapRequest* request)
{
    ...
    if (!nabtoshell_iam_check_access(&app->iam, request, "Terminal:CreateSession")) {
    ...
    const char* cmd = strlen(command) > 0 ? command : NULL;
